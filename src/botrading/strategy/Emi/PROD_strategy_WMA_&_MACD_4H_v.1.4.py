...
class Strategy:
    ...
    def apply_buy(self, bitget_data_util: BitgetDataUtil, data_frame: pandas.DataFrame) -> pandas.DataFrame:
        ...
        # -------------------------------- L O N G  ------------------------------------
        query = "(" + DataFrameColum.WMA_ASCENDING.value + " == True) and (" + DataFrameColum.MACD_CHART_ASCENDING .value + " == True)"
        ...
        query = "(" + DataFrameColum.WMA_ASCENDING.value + " == False) and (" + DataFrameColum.MACD_CHART_ASCENDING .value + " == False)"
    ...
    def apply_sell(bitget_data_util: BitgetDataUtil, data_frame: pandas.DataFrame) -> pandas.DataFrame:
        ...
        # -------------------------------- L O N G  ------------------------------------
            ...
            df_long_step_4H = filtered_data_frame.query(query)

            if df_long_step_4H.empty == False:

                df_2H = Strategy.calculate_indicators (bitget_data_util=bitget_data_util, df_master = filtered_data_frame, time_range = TimeRanges("HOUR_2"), 
                                                        num_elements_wma=2, num_elements_macd=4, num_elements_chart_macd=2)

                query = "(" + DataFrameColum.NOTE.value + " == 'CHECK_LNG') and (" + DataFrameColum.WMA_ASCENDING.value + " == False) "
                df_long_step_2H = df_2H.query(query)

                if df_long_step_2H.empty == False:
                    return Strategy.clearing_fields_sell (clean_df = filtered_data_frame) 
                
            # CONTROL CON CHART MACD 1

            query = "(" + DataFrameColum.NOTE.value + " == 'CHECK_LNG') and (" + DataFrameColum.MACD_CHART_ASCENDING.value + " == False) "
            df_long_macd_4H = filtered_data_frame.query(query)

            if df_long_macd_4H.empty == False:

                df_long_macd_2H = Strategy.calculate_indicators (bitget_data_util=bitget_data_util, df_master = df_long_macd_4H, time_range = TimeRanges("HOUR_2"), num_elements_chart_macd=4)
            
                query = "(" + DataFrameColum.NOTE.value + " == 'CHECK_LNG') and (" + DataFrameColum.MACD_CHART_ASCENDING.value + " == False) "
                df_long_macd_desc = df_long_macd_2H.query(query)

                if df_long_macd_desc.empty == False:
                    return Strategy.clearing_fields_sell (clean_df = filtered_data_frame) 
                
            # CONTROL CON CHART MACD 2

            df_long_macd_2H = Strategy.calculate_indicators (bitget_data_util=bitget_data_util, df_master = filtered_data_frame, time_range = TimeRanges("HOUR_2"), num_elements_chart_macd=3)
        
            query = "(" + DataFrameColum.NOTE.value + " == 'CHECK_LNG') and (" + DataFrameColum.MACD_CHART_ASCENDING.value + " == False) "
            df_long_macd_desc = df_long_macd_2H.query(query)

            if df_long_macd_desc.empty == False:
                df_long_macd_1H = Strategy.calculate_indicators (bitget_data_util=bitget_data_util, df_master = filtered_data_frame, time_range = TimeRanges("HOUR_1"), num_elements_chart_macd=2)
            
                query = "(" + DataFrameColum.NOTE.value + " == 'CHECK_LNG') and (" + DataFrameColum.MACD_CHART_ASCENDING.value + " == False) "
                df_long_macd_desc = df_long_macd_1H.query(query)
        ...
        # -------------------------------- S H O R T  ------------------------------------
            ...
            df_short_step_4H = filtered_data_frame.query(query)

            if df_short_step_4H.empty == False:

                df_2H = Strategy.calculate_indicators (bitget_data_util=bitget_data_util, df_master = filtered_data_frame, time_range = TimeRanges("HOUR_2"), 
                                                                num_elements_wma=2, num_elements_macd=4, num_elements_chart_macd=2)

                query = "(" + DataFrameColum.NOTE.value + " == 'CHECK_SHRT') and (" + DataFrameColum.WMA_ASCENDING.value + " == True) "
                df_short_step_2H = df_2H.query(query)

                if df_short_step_2H.empty == False:
                    return Strategy.clearing_fields_sell (clean_df = filtered_data_frame) 


            # CONTROL CON CHART MACD
            query = "(" + DataFrameColum.NOTE.value + " == 'CHECK_SHRT') and (" + DataFrameColum.MACD_CHART_ASCENDING.value + " == True) "
            df_short_macd_4H = filtered_data_frame.query(query)

            if df_short_macd_4H.empty == False:
                df_short_macd_2H = Strategy.calculate_indicators (bitget_data_util=bitget_data_util, df_master = df_short_macd_4H, time_range = TimeRanges("HOUR_2"), num_elements_chart_macd=4)
            
                query = "(" + DataFrameColum.NOTE.value + " == 'CHECK_SHRT') and (" + DataFrameColum.MACD_CHART_ASCENDING.value + " == True) "
                df_short_macd_desc = df_short_macd_2H.query(query)

                if df_short_macd_desc.empty == False:
                    return Strategy.clearing_fields_sell (clean_df = filtered_data_frame) 

            # CONTROL CON CHART MACD 2

            df_short_macd_2H = Strategy.calculate_indicators (bitget_data_util=bitget_data_util, df_master = filtered_data_frame, time_range = TimeRanges("HOUR_2"), num_elements_chart_macd=3)
        
            query = "(" + DataFrameColum.NOTE.value + " == 'CHECK_LNG') and (" + DataFrameColum.MACD_CHART_ASCENDING.value + " == False) "
            df_short_macd_desc = df_short_macd_2H.query(query)

            if df_short_macd_desc.empty == False:
                df_short_macd_1H = Strategy.calculate_indicators (bitget_data_util=bitget_data_util, df_master = filtered_data_frame, time_range = TimeRanges("HOUR_1"), num_elements_chart_macd=2)
            
                query = "(" + DataFrameColum.NOTE.value + " == 'CHECK_LNG') and (" + DataFrameColum.MACD_CHART_ASCENDING.value + " == False) "
                df_short_macd_desc = df_short_macd_1H.query(query)
