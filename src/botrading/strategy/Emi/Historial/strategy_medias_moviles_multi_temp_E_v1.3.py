""" SOLUCIÓN CON LA ESTRATEGIA DE JIMMY 
- MULTITEMPORALIDAD
- MULTICOIN
- NO USO DE SOPORTE NI RESISTENCIAS, SINO DE CONTROL DESDE EL SELL
"""
import time
...
class Strategy:
    ...
    def get_time_ranges(self) -> []:
        return ["MINUTES_5","MINUTES_15"] #, "MINUTES_15", "HOUR_1", "HOUR_4"
    ...
    def apply_buy(self, bitget_data_util: BitgetDataUtil, data_frame: pandas.DataFrame) -> pandas.DataFrame:
        ...
        for t in time_ranges:
            ...
            df_t.loc[:,self.ma_50_angle_colum] = df_t[DataFrameColum.MA_LAST_ANGLE.value]
        ...
        for time_range_name in list_time_range:
            ...
            # LONG - PASO COMPRA
            query = "(" + self.step_counter + " == 3) and (" + DataFrameColum.PRICE_CLOSE.value + " > " + self.ma_50_colum + ") and (" + self.diferencia_percentual_column + " > " + self.parallel_limit_column + ")" + " and MA_50_ANGLE > 20" 
            ...
            # SHORT - PASO COMPRA
            query = "(" + self.step_counter + " == 4) and (" + DataFrameColum.PRICE_CLOSE.value + " < " + self.ma_50_colum + ") and (" + self.diferencia_percentual_column + " > " + self.parallel_limit_column + ")" + " and MA_50_ANGLE < -20" 
    ...
    def apply_sell(self, bitget_data_util: BitgetDataUtil, data_frame: pandas.DataFrame) -> pandas.DataFrame:
        ...
        if int(settings.MAX_COIN_BUY) == len(df):
            query =  "(" + self.step_counter + " != 0)"
            df_reset = data_frame.query(query)

            if df_reset.empty == False:
                df_reset.loc[:, self.step_counter] = 0
                return df_reset
        ...
        if df.empty:
            ...
            for ind in df.index:
                ...
                rsi_ascending_count = 2
                config_stock_rsi = ConfigSTOCHrsi(longitud_stoch= 14, longitud_rsi= 14, smooth_k = 3, smooth_d = 3)
                df = bitget_data_util.updating_stochrsi(config_stoch_rsi = config_stock_rsi, data_frame = df, prices_history_dict = prices_history, ascending_count = rsi_ascending_count)
                ...
                side_type = data_frame.loc[ind,DataFrameColum.SIDE_TYPE.value]

                if side_type == 'long':
                    query = "(" + str(price_coin) + " < " + DataFrameColum.MA_LAST.value + ") and (" + DataFrameColum.MA_ASCENDING.value + " == False) and (" + DataFrameColum.RSI_STOCH_GOOD_LINE_ASCENDING.value + " == False)"
                else:
                    query = "(" + str(price_coin) + " > " + DataFrameColum.MA_LAST.value + ") and (" + DataFrameColum.MA_ASCENDING.value + " == True) and (" + DataFrameColum.RSI_STOCH_GOOD_LINE_ASCENDING.value + " == True)"
        ...
        for ind in df.index:
            cntrl_profit_crrnt = float(df.loc[ind,DataFrameColum.PERCENTAGE_PROFIT.value]) - 0.5
    ...
    def print_data_frame(self, message: str, data_frame: pandas.DataFrame, print_empty:bool=True):
        ...
        def angle(self, bitget_data_util: BitgetDataUtil, df: pandas.DataFrame):
            ...
            slope = numpy.cov(df['Close'], df['Index'])[0, 1] / np.var(df['Index'])

            # Calcular el ángulo de regresión en radianes
            angle_radians = numpy.arctan(slope)

            # Convertir radianes a grados
            return numpy.rad2deg(angle_radians)
