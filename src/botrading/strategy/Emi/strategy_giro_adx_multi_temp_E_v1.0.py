...
class Strategy:
    ...
    def apply_buy(self, bitget_data_util: BitgetDataUtil, data_frame: pandas.DataFrame) -> pandas.DataFrame:
        ...
        """
#--------------------------------------------------------- Step - Stoch_RSI ------------------------------------------------------
        prices_history = bitget_data_util.get_historial_x_day_ago_all_crypto(df_master = filtered_data_frame, time_range = TimeRanges("HOUR_1"))

        config_ma_50 = ConfigMA(length=50, type="sma")
        df_ma = bitget_data_util.updating_ma(config_ma= config_ma_50, data_frame=filtered_data_frame, prices_history_dict=prices_history)
        ...
        filtered_data_frame = DataFrameUtil.replace_rows_df_backup_with_df_for_index(filtered_df_master, df_ma)
        
        query = self.ma_50_colum + " > " + self.ma_100_colum 
        filtered_data_frame = filtered_data_frame.query(query)
        
        ascending_count = 3 #! Cambiado
        ...
        query = DataFrameColum.RSI_STOCH_CRUCE_LINE.value + " == 'BLUE_CRUCE_TOP' and " + DataFrameColum.RSI_STOCH_GOOD_LINE_LAST.value + " < 40"
        ...
        if not df_stch_rsi.empty: 
            ...
            #Copiamos la inf en el DF bckp
            filtered_df_master = DataFrameUtil.replace_rows_df_backup_with_df_for_index (df_master = filtered_df_master, df_slave = df_stch_rsi)
    ...
    def apply_sell(bitget_data_util: BitgetDataUtil, data_frame: pandas.DataFrame) -> pandas.DataFrame:
        ...
        Strategy.mark_price_exceeds_limit(data_frame = df)
        ...
        for ind in df.index:
            ...
            if df.loc[ind, DataFrameColum.NOTE_4.value] == 'Step - Stoch_RSI':
                ...
                df_op2 = df_only_coin.query(query)
        ...
        for ind in df.index:
            cntrl_profit_crrnt = float(df.loc[ind,DataFrameColum.PERCENTAGE_PROFIT.value]) - 0.2
